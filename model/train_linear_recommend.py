...
disease_df = pd.read_csv('datasets/preprocessed_recommendedfordisease.csv')
...
for disease in selected_disease:
    ...
    # Train KNN regression model with polynomial features for complexity
    degree = 3
    ...
    # Save the top 150 recommended food indices in a pickle file
    top_n = 150
    recommended_food_indices = np.argsort(predicted_distances)[:top_n]
    pickle_filename = f'saved_models/recommended_food_{disease_dict[disease]}.pkl'
    with open(pickle_filename, 'wb') as pickle_file:
        pickle.dump(recommended_food_indices, pickle_file)
